...
function Buttontag<T>(props: Props<T>) {
  const {
    action,
    value,
    setformdata,
          ...
          >
            {loader?.loader ? (
              <>
                ...
                <span
                  className={`absolute left-[42%] h-6 w-6 border-2 border-white border-t-blue-800 rounded-full animate-spin ${loader.className ? loader.className.join("") : ""}`}
                ></span>
              </>
            ) : (
              <>{value.name}</>
